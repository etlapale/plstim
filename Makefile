...
lorenceau-experiment: lorenceau-experiment.cc $(PLSTIM_HDR) $(PLSTIM_SRC)
	$(CXX) -o $@ $(CXXFLAGS) -I. -std=c++11 lorenceau-experiment.cc $(PLSTIM_SRC) `pkg-config --cflags --libs tclap` `pkg-config --cflags --libs glesv2` `pkg-config --cflags --libs egl` `pkg-config --cflags --libs libpng` `pkg-config --cflags --libs yaml-cpp` `pkg-config --cflags --libs cairomm-png-1.0` -lX11 -lrt
